	...
	if choice == "Produit":
		...
		if st.checkbox('voir dataframe'):
		   ...
		   if download:
		   	...
		   	b64 = base64.b64encode(csv.encode()).decode()  
		   	linko= f'<a href="data:file/csv;base64,{b64}" download="data_événement.csv">Download csv file</a>'
		   	st.markdown(linko, unsafe_allow_html=True)
		...
		download = st.button('télécharger')
		if download:
			csv = data3.to_csv(index=False, compression="zip")
			b64 = base64.b64encode(csv.encode()).decode()  
			linko= f'<a href="data:file/csv;base64,{b64}" download="data_lieu_intérêt.zip">Download csv file</a>'
			st.markdown(linko, unsafe_allow_html=True)		
		...
		if st.checkbox('voir dataframe'):
		   ...
		   download = st.button('télécharger')
		   if download:
		   	csv = data4.to_csv(index=False)
		   	b64 = base64.b64encode(csv.encode()).decode()  
		   	linko= f'<a href="data:file/csv;base64,{b64}" download="data_itinéraire.csv">Download csv file</a>'
		   	st.markdown(linko, unsafe_allow_html=True)		   
		...
		if mdp == "Les+tour1stes.":
			if st.checkbox('voir dataframe'):
				...
				if download:
					csv = data_erreur.to_csv(index=False)
					b64 = base64.b64encode(csv.encode()).decode()  
					linko= f'<a href="data:file/csv;base64,{csv}" download="data_erreur.csv">Download csv file</a>'
					st.markdown(linko, unsafe_allow_html=True)
